...
const api = axios.create({
    ...
    headers: {
        ...
        'Authorization': 'Bearer your_api_token_here' // Replace with actual token management
    },
});
...
api.interceptors.response.use(
    ...
    (error) => {
        if (error.response?.status === 401) {
            ...
            // You can redirect to login page or show error message here
        }
...
export interface Task {
    _id: string;
    type: 'MIXER' | 'MAKER' | 'HOLDER' | 'VOLUME' | 'SWEEP';
    name?: string;
    context: any;
    status: 'FAILED' | 'PENDING' | 'RUNNING' | 'STOPPED' | 'DELETED' | 'COMPLETED';
    startedAt?: string;
    endedAt?: string;
