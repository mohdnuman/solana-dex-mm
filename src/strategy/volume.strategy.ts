        ...
        try {
            while (true) {
                ...
                for (const trade of trades) {
                    ...
                    amount = type === tradeTypeEnum.BUY ? amount : await dexInterface.getTokenAmountToSellToGetGivenSolAmount(config.dex, config.poolAddress, amount)

                    const wallet =
                        type === tradeTypeEnum.BUY
                            ? this.configProvider.getRandomWalletFromGroup(
                                config.volumeStrategy.walletGroupName,
