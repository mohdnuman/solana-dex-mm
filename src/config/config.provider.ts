...
import validatorLib from "../lib/validator.lib";
...
export default class ConfigProvider {
    ...
    config: {
        ...
        makerStrategy: {
            ...
            amountToSwap: number;
            makerStratWalletGroupName: string;
            isEnabled: boolean;
        };
    };
    ...
    private walletBalances: Map<
        string,
        { solBalance: number; tokenBalance: number }
    > = new Map();
    ...
    constructor(filePath: string) {
        ...
        validatorLib.validateConfig(config);

        this.config = config;

        if (_.isEmpty(config.walletGroups)) {
            this.createWalletGroup("default", 10);
            loggerLib.logInfo({
                message:
                    "No wallet groups found in config. Created default wallet group with 10 wallets.",
            })
        }

        const walletGroupNames = this.getWalletGroupNames();
        if (config.volumeStrategy.walletGroupName && !walletGroupNames.includes(config.volumeStrategy.walletGroupName)) {
            throw new Error(`Wallet group '${config.volumeStrategy.walletGroupName}' for volume strategy not found`);
        }

        if (config.makerStrategy.walletGroupName && !walletGroupNames.includes(config.makerStrategy.walletGroupName)) {
            throw new Error(`Wallet group '${config.makerStrategy.walletGroupName}' for maker strategy not found`);
        }
    ...
    getWalletGroups=() => {
        try {
            const config = this.getConfig();
            return config.walletGroups.map((group: WalletGroup) => ({
                name: group.name,
                wallets: group.wallets.map((wallet: Wallet) => ({
                    ...wallet,
    ...
    createWalletGroup = (groupName: string, walletCount: number): WalletGroup => {
        ...
        const wallets: Wallet[] = _.range(walletCount).map(() =>
            solanaLib.generateWallet(),
        );
        ...
        this.updateConfig({
            walletGroups: [...config.walletGroups, newWalletGroup],
        });
        ...
        try {
            setInterval(async () => {
                try {
                    ...
                    const allWallets = walletGroups.flatMap(
                        (group: WalletGroup) => group.wallets,
                    );

                    if (_.isEmpty(allWallets)) return;

                    const walletChunks: Wallet[][] = _.chunk(
                        allWallets,
                        MAX_WALLETS_PER_MULTI_CALL,
                    );

                    const solBalancesPromises = walletChunks.map(
                        (walletChunk: Wallet[]) =>
                            solanaLib.getBatchSolBalance(
                    ...
                    const tokenBalancesPromises = walletChunks.map(
                        (walletChunk: Wallet[]) =>
    ...
    addWalletsToGroup = (groupName: string, walletCount: number): WalletGroup => {
        const config = this.getConfig();
        const groupIndex = config.walletGroups.findIndex(
            (group: WalletGroup) => group.name === groupName,
        );
        ...
        const walletsWithBalances: any[] = updatedGroup.wallets.map(
            (wallet: Wallet) => ({
                ...wallet,
                solBalance: this.walletBalances.get(wallet.address)?.solBalance || 0,
                tokenBalance:
                    this.walletBalances.get(wallet.address)?.tokenBalance || 0,
            }),
        );
