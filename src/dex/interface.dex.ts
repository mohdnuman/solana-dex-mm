...
async function buy(dex: string, address: string, amountOfSol: number, walletPrivateKey: string): Promise<string> {
    try {
        if (
            _.isNil(dex) ||
            _.isNil(address) ||
...
async function sell(dex: string, address: string, amountOfTokens: number, walletPrivateKey: string): Promise<string> {
    try {
        if (
            _.isNil(dex) ||
            _.isNil(address) ||
