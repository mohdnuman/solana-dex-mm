...
async function buy(poolAddress: string, amountInSol: number, walletPrivateKey: string): Promise<string> {
    try {
        if (
...
async function sell(poolAddress: string, amount: number, walletPrivateKey: string): Promise<string> {
    try {
        if (
            _.isEmpty(poolAddress) ||
            _.isNil(amount) ||
