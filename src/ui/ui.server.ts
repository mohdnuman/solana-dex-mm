...
export default class UiServer {
    constructor(configProvider: ConfigProvider) {
        ...
        app.post("/api/config", (req, res) => {
            try {
                const allowedFields = ["bias", "tradesPerCycle", "volumePerMinute", "volumeStratWalletGroupName"];
        ...
        app.get("/api/wallet-groups", (req, res) => {
            try {
                ...
                const groupsWithStats = walletGroups.map((group: any) => ({
                    ...
                    totalSolBalance: group.wallets.reduce((sum: number, wallet: any) => sum + (wallet.solBalance || 0), 0),
                    totalTokenBalance: group.wallets.reduce((sum: number, wallet: any) => sum + (wallet.tokenBalance || 0), 0),
        ...
        app.post("/api/wallet-groups", (req, res) => {
            try {
                ...
                if (!walletCount || typeof walletCount !== 'number' || walletCount < 1 || walletCount > 100) {
                    return res.status(400).json({error: "Wallet count must be a number between 1 and 100"});
                }

                const trimmedGroupName = groupName.trim();
                const newGroup = configProvider.createWalletGroup(trimmedGroupName, walletCount);
        ...
        app.get("/api/wallet-groups/:groupName/export", (req, res) => {
            try {
                ...
                const config = configProvider.getConfig();
                const group = config.walletGroups.find((g: any) => g.name === groupName);
                ...
                const privateKeys = group.wallets.map((wallet: any) => wallet.privateKey);
                const privateKeysText = privateKeys.join('\n');

                // Set headers for file download
                res.setHeader('Content-Type', 'text/plain');
                res.setHeader('Content-Disposition', `attachment; filename="${groupName}_private_keys.txt"`);
        ...
        app.post("/api/wallet-groups/:groupName/add-wallets", (req, res) => {
            try {
                ...
                if (!walletCount || typeof walletCount !== 'number' || walletCount < 1 || walletCount > 50) {
                    return res.status(400).json({error: "Wallet count must be a number between 1 and 50"});
                }

                const updatedGroup = configProvider.addWalletsToGroup(groupName, walletCount);
                ...
                return res.status(200).json({
                    ...
                    group: {
                        ...
                        totalSolBalance: updatedGroup.wallets.reduce((sum: number, wallet: any) => sum + (wallet.solBalance || 0), 0),
                        totalTokenBalance: updatedGroup.wallets.reduce((sum: number, wallet: any) => sum + (wallet.tokenBalance || 0), 0)
                    }
                });
            } catch (error) {
                loggerLib.logError(error);
                ...
                return res.status(500).json({error: "Failed to add wallets to group"});
            }
        });
